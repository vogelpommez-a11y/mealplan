...
(function (global) {
  ...
  // unit ist "g" oder "ml" und dient nur der Beschriftung - gerechnet wird mit grams.
  ...
  function offServingSize(p) {
    ...
    if (multi) {
      ...
      if (count > 0 && grams > 0) return { grams, count, serving: !!serv, unit: multi[3].toLowerCase() };
    }
    ...
    if (piece) {
      ...
      if (count > 0 && grams > 0) return { grams, count, serving: !!serv, unit: piece[3].toLowerCase() };
    }
    ...
    if (plain) {
      ...
      // Nach dem Hochrechnen ist aus "1 l" 1000 ml geworden, aus "1 kg" 1000 g - die
      // Einheit faellt dabei auf ihre kleine Schwester zurueck, damit die Anzeige sie
      // richtig beschriftet.
      if (grams > 0) return { grams, count: null, serving: !!serv, unit: (unit === "l" || unit === "ml") ? "ml" : "g" };
    }
    return null;
  }
  async function fetchOffNutrition(code) {
    ...
    // 404 ist bei Open Food Facts die normale Antwort auf einen Code, den die Datenbank
    // nicht kennt - und das ist beim Scannen der HAEUFIGSTE Fall, nicht der Ausnahmefall.
    // Als Fehler geworfen landete er beim Aufrufer im catch-Zweig und wurde dem Nutzer als
    // "Suche nicht moeglich (offline?)" gemeldet, obwohl seine Verbindung tadellos stand.
    // Ein unbekanntes Produkt sieht ab hier genauso aus wie ein leerer Treffer: null.
